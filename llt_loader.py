...
Uses ANT standard XML. Key rules:
  - All backslashes MUST be doubled in values: C:\\\\path\\\\file.ext
...
PERSISTENT CONFIG: ~/.llt_loader_config.json
  ...
  File names stored separately — change folder → all paths auto-update.
...
def esc(path: str) -> str:
    ...
    - Doubles backslashes per LLT User Guide CAUTION (Section 7.1)
    """
    path = path.replace("&", "&amp;")
    path = path.replace('"', "&quot;")
    return path.replace("\\", "\\\\")
...
def build_xml(files: list[dict], port: str, opts: dict) -> str:
    ...
    # ── Pre-connect: enable/disable options ──
    if opts.get("ignore_errors"):
        t += f'{I}<jllt.commandLineTask\n{I}        action = "enable"\n{I}        option = "ignore errors"/>\n'
    if not opts.get("force_uppercase", True):
        t += f'{I}<jllt.commandLineTask\n{I}        action = "disable"\n{I}        option = "force uppercase"/>\n'
    if opts.get("result_file"):
        rp = esc(os.path.join(opts.get("out_dir", tempfile.gettempdir()), f"result_{port or 'auto'}.log"))
        t += f'{I}<jllt.commandLineTask\n{I}        action = "enable"\n{I}        option = "result file"\n{I}        target = "{rp}"/>\n'

    # ── List terminals (can be called before or after connect) ──
    ...
    if opts.get("query_info"):
        ...
        t += f'{I}<jllt.commandLineTask\n{I}        action = "query"\n{I}        property = "Manufacturing Code"\n{I}        target = "{esc(os.path.join(od, f"MfgCode_{lbl}.txt"))}"/>\n'
        t += f'{I}<jllt.commandLineTask\n{I}        action = "query"\n{I}        property = "Islero Serial Number"\n{I}        target = "{esc(os.path.join(od, f"Serial_{lbl}.txt"))}"/>\n'

    # ── Clean terminal ──
    if opts.get("clean_before"):
        t += f'{I}<jllt.commandLineTask\n{I}        action = "clean"'
        if opts.get("uninstall_packages"):
            t += f'\n{I}        option = "uninstall packages"'
        t += '/>\n'

    # ── Downloads (the main payload) ──
    for fi in files:
        t += f'{I}<jllt.commandLineTask\n{I}        action = "download"\n{I}        source = "{esc(fi["path"])}"'
        if not opts.get("auto_dest"):
            t += f'\n{I}        target = "{fi["dest"]}"'
        t += '/>\n'
    ...
    if opts.get("upload_config"):
        ...
        if opts.get("range") == "Telium Tetra":
            src = "/SYST/HTERMINAL.INI"
            dest_name = f"HTERMINAL_{lbl}.INI"
        else:
            src = "/HOST/APPRESET.DIA"
            dest_name = f"APPRESET_{lbl}.DIA"
        ud = esc(os.path.join(opts.get("out_dir", tempfile.gettempdir()), dest_name))
        t += f'{I}<jllt.commandLineTask\n{I}        action = "upload"\n{I}        source = "{src}"\n{I}        target = "{ud}"/>\n'

    # ── Activity change ──
    if opts.get("activity"):
        t += f'{I}<jllt.commandLineTask\n{I}        action = "activity"\n{I}        mode = "{opts["activity"]}"/>\n'
...
class LLTApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.folder_path  = tk.StringVar(value=cfg.get("folder_path", ""))
        ...
        self.force_upper  = tk.BooleanVar(value=cfg.get("force_uppercase", True))
        ...
        self.saved_names: list[str] = cfg.get("saved_files", [])
    ...
    def _save_cfg(self):
        save_config(dict(
            llt_path=self.llt_path.get(), folder_path=self.folder_path.get(),
    ...
    def _build_ui(self):
        ...
        r2 = ttk.Frame(pf); r2.pack(fill=tk.X)
        ttk.Label(r2, text="Package Folder:", width=16, anchor="e").pack(side=tk.LEFT)
        fe = ttk.Entry(r2, textvariable=self.folder_path)
        fe.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=6)
        fe.bind("<Return>",   lambda _: self._rebuild_tree())
        fe.bind("<FocusOut>", lambda _: self._rebuild_tree())
        ttk.Button(r2, text="Browse", command=self._br_folder).pack(side=tk.LEFT)
        ttk.Button(r2, text="Scan", command=self._scan).pack(side=tk.LEFT, padx=(4, 0))
        ...
        ttk.Label(pf, text="Change folder → all file paths update. "
        ...
        self.tree.heading("path", text="Full Path (auto-built)")
        ...
        ttk.Button(bf, text="Preview XML", command=self._preview).pack(side=tk.LEFT, padx=10)
        ttk.Button(bf, text="Export Scripts…", command=self._export).pack(side=tk.LEFT)
        ttk.Button(bf, text="Open Output", command=self._open_out_dir).pack(side=tk.LEFT, padx=10)
    ...
    def _br_folder(self):
        p = filedialog.askdirectory(title="Package Folder", initialdir=self.folder_path.get() or None)
        if p: self.folder_path.set(os.path.normpath(p)); self._rebuild_tree()
    ...
    def _br_files(self):
        ...
        files = filedialog.askopenfilenames(
            title="Select Files", initialdir=self.folder_path.get() or None,
            filetypes=[("LLT files", exts), ("All", "*.*")])
        added = 0
        for fp in files:
            fn = os.path.basename(fp)
            if fn not in self.saved_names:
                self.saved_names.append(fn); added += 1
        if added:
            self._rebuild_tree(); self._log(f"+ {added} file(s)", "ok")

    def _scan(self):
        """Scan folder for all loadable files and replace file list."""
        folder = self.folder_path.get()
        if not folder or not os.path.isdir(folder):
            self._log("Package Folder not set or not found — set a valid path first", "warn"); return
        try:
            entries = sorted(os.listdir(folder))
        except PermissionError as e:
            self._log(f"Cannot read folder: {e}", "err"); return
        except OSError as e:
            self._log(f"Folder error: {e}", "err"); return
        self.saved_names.clear()
        for fn in entries:
            if not os.path.isfile(os.path.join(folder, fn)):
                continue
            ext = os.path.splitext(fn)[1].upper()
            if ext in ALL_EXT or is_catalogue(ext):
                self.saved_names.append(fn)
        self._rebuild_tree()
        count = len(self.saved_names)
        self._log(f"Scanned: {count} file(s)", "ok" if count else "warn")

    def _add_manual(self):
        fn = self.file_entry.get().strip()
        if not fn: return
        ext = os.path.splitext(fn)[1].upper()
        if ext not in ALL_EXT and not is_catalogue(ext):
            self._log(f"Unsupported: {ext}", "err"); return
        if fn in self.saved_names:
    ...
    def _rm_selected(self):
        ...
        for item in self.tree.selection():
            ...
            if name in self.saved_names: self.saved_names.remove(name)
    ...
    def _rebuild_tree(self):
        """Rebuild treeview from saved_names + folder_path + range."""
        for item in self.tree.get_children(): self.tree.delete(item)
        folder = os.path.normpath(self.folder_path.get()) if self.folder_path.get() else ""
        rng = self.range_var.get()
        for fn in self.saved_names:
            ext = os.path.splitext(fn)[1].upper()
            fp = os.path.join(folder, fn) if folder else fn
    ...
    def _preview(self):
        """Show generated XML for first active terminal in a popup."""
        files = self._get_files()
        if not files: self._log("No files", "err"); return
        active = [t for t in self.terminals if t.get("enabled")]
        port = active[0].get("port", "") if active else ""
        opts = self._opts()
        xml = build_xml(files, port, opts)
        bat = build_batch(os.path.normpath(self.llt_path.get()), port, f"load_{port or 'auto'}.xml", opts)

        win = tk.Toplevel(self.root)
        win.title(f"Preview — {port or 'auto'}"
                  + (f" (+ {len(active)-1} more)" if len(active) > 1 else ""))
        win.geometry("750x550"); win.configure(bg=Theme.BG)
        ...
        txt.insert(tk.END, f"{'='*60}\n  XML for {port or 'auto'}\n{'='*60}\n\n{xml}\n\n")
        txt.insert(tk.END, f"{'='*60}\n  Batch\n{'='*60}\n\n{bat}\n")
        if len(active) > 1:
            txt.insert(tk.END, f"\n\n--- NOTE: {len(active)} terminals enabled. "
                       f"Each gets its own XML with its own <connect device=.../> ---\n")
